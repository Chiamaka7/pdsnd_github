...
def get_filters():
    ...
    """
    # Welcome graphics
    pattern(3, 'Hi! Let\'s explore some US bikeshare data!')
    # get user input for city (chicago, new york city, washington).
    while True:
        user_city = input('Would you want to see data for Chicago, New York City, or Washington?\n').lower()
        if user_city not in CITY_DATA:
            ...
            pattern(2, f'You are interested in: {user_city.title()}')
    ...
    if user_filter == 'both':
        ...
        while True:
            ...
            if user_day not in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']:
                ...
                pattern(2, f'showing data for filter: {user_city}---{user_filter} (by Month:by day)--> {user_month},'
                           f' {user_day}')
                break
    ...
    # get user input for month (all, january, february, ... , june)
        ...
        while True:
            ...
            if user_month not in ['january', 'february', 'march', 'april', 'may', 'june']:
                ...
                pattern(2, f'showing data with filter: {user_city}---{user_filter}---> {user_month}')
                break
    ...
    # get user input for day of week (all, monday, tuesday, ... sunday)
        ...
        while True:
            ...
            if user_day not in ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']:
                ...
                pattern(2, f'showing data with filter: {user_city}---{user_filter}---> {user_day}')
                break
...
def load_data(city, month, day):
    """
    Loads data for the specified city and filters by user-defined month and day.
...
def time_stats(df):
    ...
    print('\n wait a moment.... Loading the data')
...
def station_stats(df):
    ...
    pattern(2, 'Calculating The Most Popular Stations and Trip...')
    start_time = time.time()
...
def trip_duration_stats(df):
    ...
    pattern(2, 'Calculating Trip Duration...')
    start_time = time.time()
...
def user_stats(df):
    ...
    pattern(2, 'Calculating User Stats...')
    start_time = time.time()
    ...
    while True:
        ...
        while True:
            ...
            if restart.lower() not in ['yes', 'no']:
                ...
                break
        if restart.lower() == 'no':
            pattern(5, 'Thank you for your time')
            break
